...
export default function PlayerProjections(props) {
  ...
  const changeField = (field, posTab, playerId, value) => {
    console.log(field, posTab, playerId, value);
    let playerDataCopy = { ...props.globalPlayerData };
    playerDataCopy[posTab] = playerDataCopy[posTab].map(player => {
    ...
    //playerDataCopy[posTab]
    console.log(playerDataCopy);
    props.setGlobalPlayerData(playerDataCopy);
  };

  const getLockCount = (lockCountObject, position) => {
    switch (position) {
      case "QB":
        return lockCountObject["QB"];
      case "RB":
      case "WR":
      case "TE":
        return (
          lockCountObject["RB"] + lockCountObject["WR"] + lockCountObject["TE"]
        );
      case "DST":
        return lockCountObject["DST"];
      default:
        return false;
    }
  };
  ...
  const lockPlayer = (posTab, playerId) => {
    let lockCount = getLockCount(props.positionLocks, posTab);
    let inactivatePlayers = false;
    if (posTab === "QB" || posTab === "DST") {
      inactivatePlayers = true;
    } else if (posTab === "RB") {
      if (props.positionLocks["RB"] === 2) {
        inactivatePlayers = true;
      } else if (lockCount === 6) {
        inactivatePlayers = true;
      }
    } else if (posTab === "WR") {
      if (props.positionLocks["WR"] === 3) {
        inactivatePlayers = true;
      } else if (lockCount === 6) {
        inactivatePlayers = true;
      }
    } else {
      if (props.positionLocks["TE"] === 1) {
        inactivatePlayers = true;
      } else if (lockCount === 6) {
        inactivatePlayers = true;
      }
    ...
    playerDataCopy[posTab] = playerDataCopy[posTab].map(player => {
      if (player.player_id === playerId) {
        ...
        if (inactivatePlayers && !player["locked"]) {
          player["active"] = false;
        }
        return player;
      }
    ...
    //playerDataCopy[tab]
    props.setPositionLocks({
      ...props.positionLocks,
      [posTab]: props.positionLocks[posTab] + 1
    });
    props.setGlobalPlayerData(playerDataCopy);
  };
